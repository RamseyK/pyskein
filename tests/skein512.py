...
R = [[46, 36, 19, 37], [33, 27, 14, 42], [17, 49, 36, 39], [44, 9, 54, 56], [39, 30, 34, 24], [13, 50, 10, 17], [25, 29, 39, 43], [8, 35, 56, 22]]
...
def skein512(init=b"", digest_bits=512, key=b"", pers=b"", public_key=b"", key_id=b"", nonce=b"", tree=None):
...
def tree_hash(g, msg, leaf_size, children, max_level):
    if msg:
        ...
        blocks = [b""]
    blocks = [ubi(g, block, (i * leaf_size) | (1 << 112) | (48 << 120)) for i, block in enumerate(blocks)]
    level = 2
    while (level < max_level) and (len(blocks) > 1):
        ...
        blocks = [ubi(g, block, (64 * i * children) | (level << 112) | (48 << 120)) for i, block in enumerate(blocks)]
        level += 1
    if len(blocks) == 1:
        return blocks[0]
    else:
